...
#
       for i in 1:num_segments
           ...
           # Enforce level continuity for stateless models.  For the first segment
           # the target is the first observed value (so the simulation starts at
           # y[1]); for later segments the target is the last simulated value of
           # the previous segment.
           if needs_level_continuity(model_manager)
               target = (i == 1) ? data[1, idx_start] : level_target
               seg_pars = adjust_segment_params_for_continuity(
                   model_manager, seg_pars, target; t_start=idx_start
               )
           end
           ...
           # Update initial condition / level target for the next segment
           u0 = update_initial_condition(model_manager, sim_data)
           if needs_level_continuity(model_manager)
               level_target = continuity_target_level(model_manager, sim_data)
           end
       end
    else
        segment_data = data
        ...
        if needs_level_continuity(model_manager)
            seg_pars = adjust_segment_params_for_continuity(
                model_manager, seg_pars, data[1, 1]; t_start=idx_start
            )
        end
