#!/usr/bin/env julia
# Task D — Uncertainty quantification for COVID-19 change points and parameters.
# Uses the zero-penalty best fit from Task A.
#   1. Parametric bootstrap confidence intervals for parameters (fixed CPs).
#   2. Profile loss / local perturbation intervals for change-point dates.
...
const B_BOOT = 30             # bootstrap replicates
...
function refit_params(cps, data, x0; use_lbfgs=true)
    ...
    function wrapped_obj(chrom)
        return Mica.objective_function(chrom, cps, parnames, n_global, n_segment_specific,
                                       ModelManager(ODEModelSpec(example_ode_model, x0_use, u0_full, (0.0, Float64(n)))),  # dummy initial chromosome not used by model
                                       (obs, sim) -> loss_abs_log(sim, obs),
                                       data)
    end
    # We need a ModelManager that actually uses the passed chromosome. The helper above uses model_manager.
    # Better create a single model_manager outside.
end

# Re-define using a proper model manager
...
function bootstrap_parameters()
    println("[Task D] Simulating best fit for bootstrap...")
    ...
    Random.seed!(42)
    for b in 1:B_BOOT
        # Resample residuals over time (independent across channels)
    ...
    println("[Task D] Parameter bootstrap saved.")
    return ci_df
end

# ---------- 2. CP profile / perturbation intervals ----------
function cp_profile_loss()
    println("[Task D] Computing CP profile losses...")
    ...
    # Approximate CIs: all candidate CPs with delta_loss <= 3.84 (heuristic)
    ...
    println("[Task D] CP profile intervals saved.")
    return ci_df
end
...
function write_report(ci_params, ci_cps)
    lines = String[]
    push!(lines, "# Task D — Uncertainty quantification for COVID-19 parameters and change points")
    push!(lines, "")
    push!(lines, "**Method:**")
...
function main()
    ...
    println("[Task D] Done. Outputs in $OUT_DIR")
end
