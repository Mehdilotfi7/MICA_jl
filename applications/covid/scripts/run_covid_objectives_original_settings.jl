#!/usr/bin/env julia
# ============================================================
# Run MICA on the COVID-19 Germany example using the original
# example's GA settings (populationSize=100, 1000 iterations) and
# compare four objective choices:
#   - :penalty with zero penalty   ("without penalty")
#   - :bic
#   - :mdl
#   - :aic
#
# Based on Mica.jl/examples/Covid-model/_example_covid_ODE.jl
# Output: results/covid_objectives_origset_summary.json
#         results/covid_detected_cps_origset_<obj>.csv
#         results/covid_params_origset_<obj>.csv
# ============================================================
...
# Optional second CLI argument: output-directory suffix (default "pop150")
out_suffix = length(ARGS) >= 2 ? ARGS[2] : "pop150"
const OUT_DIR = joinpath(@__DIR__, "..", "..", "..", "results_$(out_suffix)")
mkpath(OUT_DIR)
...
# ---------- MICA settings (match original example) ----------
...
# Original GA settings: populationSize=150 and 1000 iterations.
...
opt = EvolutionaryOptimizer(ga, options=Evolutionary.Options(show_trace=false, iterations=1000), seed=1234)

# ---------- run four objective variants ----------
# For :penalty the function must declare the argument name the API passes (here `n`).
zero_penalty(p, n) = 0.0
dummy_penalty(p, n_obs) = 0.0

all_runs = Dict(
    "none" => (:penalty, zero_penalty),
    "bic"  => (:bic,     dummy_penalty),
    "mdl"  => (:mdl,     dummy_penalty),
    "aic"  => (:aic,     dummy_penalty),
)

# Optional command-line objective (e.g. julia script.jl bic pop150)
requested = length(ARGS) >= 1 ? ARGS[1] : "all"
if requested == "all"
    todo = collect(keys(all_runs))
else
    haskey(all_runs, requested) || error("Unknown objective: $requested. Choose from $(join(keys(all_runs), ", "))")
    todo = [requested]
end

summary_file = joinpath(OUT_DIR, "covid_objectives_origset_summary.json")
summary = isfile(summary_file) ? JSON.parsefile(summary_file) : Dict{String,Any}[]
done = Set(r["objective"] for r in summary)

for label in todo
    label in done && (println("Objective $label already present in summary; skipping."); continue)
    obj, pen = all_runs[label]
    println("\nRunning COVID objective = $label")
    Random.seed!(1234)
    t0 = time()
    ...
    # No boundary filter: keep all accepted change points (matches original example)
    ...
    push!(summary, rec)
    # keep summary sorted by objective for readability
    sort!(summary, by=x->x["objective"])
    open(summary_file, "w") do f
        JSON.print(f, summary, 2)
    end
    println("  #CPs = $(length(detected_cp)), CPs = $detected_cp, time = $(round(elapsed,digits=1))s")
end
println("\nSummary saved to $(joinpath(OUT_DIR, "covid_objectives_origset_summary.json"))")
