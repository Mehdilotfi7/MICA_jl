# ============================================================
# Run MICA on the COVID-19 Germany example with BIC/MDL/AIC.
#
# Based on Mica.jl/examples/Covid-model/_example_covid_ODE.jl
# Output: results/covid_detected_cps_<obj>.csv, results/covid_params_<obj>.csv,
#         results/covid_objectives_summary.json
# ============================================================
...
for obj in [:bic, :mdl, :aic]
    println("\nRunning COVID with objective = $obj")
    Random.seed!(1234)
    t0 = time()
    ...
    CSV.write(joinpath(OUT_DIR, "covid_detected_cps_$(obj).csv"),
    ...
    CSV.write(joinpath(OUT_DIR, "covid_params_$(obj).csv"),
...
open(joinpath(OUT_DIR, "covid_objectives_summary.json"), "w") do f
    JSON.print(f, summary, 2)
end
println("\nCOVID objectives summary saved to $(joinpath(OUT_DIR, "covid_objectives_summary.json"))")
