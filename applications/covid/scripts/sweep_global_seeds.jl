#!/usr/bin/env julia
# ============================================================
# Sweep global RNG seeds for the no-per-optimization-seed case.
# Keeps populationSize=150, no boundary filter, original update_bounds!,
# and parallel candidate evaluation. For each global seed, workers are
# reset to the same seed before the run so the result is reproducible.
# ============================================================
...
# Global seeds to test
global_seeds = length(ARGS) >= 1 ? parse.(Int, split(ARGS[1], ",")) : [1, 42, 2025, 9999, 12345]

OUT_DIR = joinpath(@__DIR__, "..", "..", "..", "results_global_seed_sweep")
mkpath(OUT_DIR)

results = Vector{Dict{String,Any}}()

for gseed in global_seeds
    println("\n=== global seed = $gseed ===")
    # Reset RNG on main and all workers
    Random.seed!(gseed)
    @everywhere Random.seed!($gseed)
    ...
    opt = EvolutionaryOptimizer(ga,
            options=Evolutionary.Options(show_trace=false, iterations=iterations), seed=nothing)
    ...
    open(joinpath(OUT_DIR, "global_seed_sweep.json"), "w") do f
        JSON.print(f, results, 2)
    end
    CSV.write(joinpath(OUT_DIR, "cps_seed$(gseed).csv"), DataFrame(cp=detected_cp))
end

println("\nSweep complete. Results saved to $OUT_DIR")
for r in results
    println("seed=$(r["global_seed"]): $(r["n_cps"]) CPs: $(r["cps"])")
end
