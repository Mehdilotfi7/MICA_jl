#!/usr/bin/env julia
# Task F — Post-process detected change points: test redundancy and local relocation.
# Starts from the zero-penalty Task A result and uses fast LBFGS refits.
...
const OUT_DIR = joinpath(@__DIR__, "..", "..", "..", "outputs",  "TASK_F")
mkpath(OUT_DIR)
...
function main()
    println("[Task F] Best-fit CPs: $cps_best")
    best_loss, _ = refit(cps_best, params_best)
    println("[Task F] Best loss = $(round(best_loss, digits=2))")
    ...
    println("[Task F] Refined CP set (drop Δloss < 5%): $refined_cps")

    # 2. Optional local relocation: for each CP, scan ±5 days
    ...
    println("[Task F] Refined + relocated CP set: $refined_relocated")
    ...
    # Report
    lines = String[]
    push!(lines, "# Task F — Post-processing: redundancy and local relocation")
    push!(lines, "")
    ...
    println("[Task F] Report saved to $(OUT_DIR)/report.md")
end
