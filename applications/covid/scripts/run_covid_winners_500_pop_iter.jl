#!/usr/bin/env julia
# ============================================================
# Run the 5 COVID-19 MICA winner configurations on a 500-day window.
#
# Winners:
#   bic                  -> objective :bic, equal-weight log loss
#   mdl                  -> objective :mdl, equal-weight log loss
#   invmean_boxcox_aic   -> variant invmean, loss boxcox, penalty aic
#   invstd_boxcox_aic    -> variant invstd,  loss boxcox, penalty aic
#   invstd_sqrt_aic      -> variant invstd,  loss sqrt,   penalty aic
#
# Usage:
#   julia -p 20 run_covid_winners_500.jl [out_suffix]
#
# Output: revision/outputs/TASK_G_500/winners_500_<suffix>/
# ============================================================
...
function run_winner(label::String, variant::Union{String,Nothing}, loss_label::String, penalty_label::String)
    OUT_DIR = joinpath(OUT_ROOT, "winners_500_$(label)_pop_iter")
    mkpath(OUT_DIR)
...
function main()
    ...
    winners = [
        ("bic", nothing, "log", "bic"),
        ("mdl", nothing, "log", "mdl"),
        ("invmean_boxcox_aic", "invmean", "boxcox", "aic"),
        ("invstd_boxcox_aic", "invstd", "boxcox", "aic"),
        ("invstd_sqrt_aic", "invstd", "sqrt", "aic"),
    ]
    ...
    if single_label === nothing
        summary_file = joinpath(OUT_ROOT, "winners_500_pop_iter_summary_$(out_suffix).json")
