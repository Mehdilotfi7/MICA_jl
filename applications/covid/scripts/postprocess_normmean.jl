#!/usr/bin/env julia
# Post-process the normmean run: compute comparable losses and update
# scaling-variant comparison tables / report.
...
const NORM_DIR    = joinpath(TASK_B_DIR, "results_normmean")
...
function normmean_loss(sim)
    total = 0.0
    for k in 1:5
        r = data_indices[k]
        total += sum(abs, sim[r, :] ./ channel_means[k] .- data_CP[k, :] ./ channel_means[k])
    end
    return total
end

# ---------- load normmean result ----------
summary_file = joinpath(NORM_DIR, "summary.json")
summary = JSON.parsefile(summary_file)
cps = Int.(summary["cps"])
params = Float64.(CSV.read(joinpath(NORM_DIR, "covid_params_origset_normmean.csv"), DataFrame).value)

# simulate and compute losses
sim = simulate_with_cps(cps, params)
ll_equal = equal_weight_log_loss(sim)
ll_norm  = normmean_loss(sim)

# update summary.json
summary["loss"] = ll_equal
summary["normmean_loss"] = ll_norm
summary["equal_weight_log_loss"] = ll_equal
open(summary_file, "w") do f
    JSON.print(f, summary, 2)
end
println("Updated $(summary_file)")
println("  equal-weight log-loss = $(ll_equal)")
println("  normmean (native) loss = $(ll_norm)")

# ---------- update scaling-variant comparison ----------
...
if isfile(comp_csv)
    ...
    if !hasproperty(comp, :equal_weight_log_loss)
        comp.equal_weight_log_loss = fill(NaN, nrow(comp))
    end
    row = findfirst(==("normmean"), comp.variant)
    if row !== nothing
        comp[row, :final_loss] = ll_norm
        comp[row, :equal_weight_log_loss] = ll_equal
    else
        push!(comp, ("normmean", length(cps), join(cps, ";"), ll_norm, summary["time_seconds"], ll_equal))
    end
    ...
    println("Updated $(comp_csv) and $(comp_md)")
end
...
if isfile(report_file)
    ...
    # Replace the old comparison table with an updated one
    table_header = "## 3. Change-point comparison across loss variants"
    if occursin(table_header, report)
        # build new block
        new_block = String[]
        ...
        # replace from header up to "### Discussion"
        idx1 = findfirst(table_header, report)[1]
        idx2 = findfirst("### Discussion", report)[1]
        report = report[1:idx1-1] * join(new_block, "\n") * "\n" * report[idx2:end]
    end
    # Add normmean-specific discussion sentence if not already present
    if !occursin("normmean variant", report)
        # append after Recommendation section? Simpler: append a short paragraph before final output files.
        norm_note = """

### Normalized-raw (normmean) variant

The `normmean` variant divides each channel by its mean before computing raw absolute errors. Under its own objective it places all eight detected change points in the vaccination rollout window (indices 310–390, native loss = $(round(ll_norm, digits=4))). When evaluated with the common equal-weight log-transformed loss, the same fit yields $(round(ll_equal, digits=2)), i.e. a substantially worse fit than the baseline (≈699). This confirms that the scaling/normalisation choice materially changes the inferred change points.
"""
        # insert before "## 5. Output files"
        report = replace(report, "## 5. Output files" => norm_note * "## 5. Output files")
    end
    # Update output files list to include normmean
    if !occursin("results_normmean", report)
        report = replace(report,
            "```\napplications/covid/scripts/../../../../revision/outputs/TASK_B\n" =>
            "```\noutputs/TASK_B\n")  # updated path for public repo
        # More robust: insert before closing ``` of output files block
        if occursin("├── results_invstd/", report)
            report = replace(report,
                "├── results_invstd/\n│   ├── covid_detected_cps_origset_invstd.csv\n│   ├── covid_params_origset_invstd.csv\n│   ├── summary.json\n│   ├── covid_visualization_origset_invstd_log.png\n│   └── covid_visualization_origset_invstd_raw.png\n```" =>
                "├── results_invstd/\n│   ├── covid_detected_cps_origset_invstd.csv\n│   ├── covid_params_origset_invstd.csv\n│   ├── summary.json\n│   ├── covid_visualization_origset_invstd_log.png\n│   └── covid_visualization_origset_invstd_raw.png\n├── results_normmean/\n│   ├── covid_detected_cps_origset_normmean.csv\n│   ├── covid_params_origset_normmean.csv\n│   ├── summary.json\n│   ├── covid_visualization_origset_normmean_log.png\n│   └── covid_visualization_origset_normmean_raw.png\n```")
        end
    end
    write(report_file, report)
    println("Updated $(report_file)")
end
