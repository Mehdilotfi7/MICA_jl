#!/usr/bin/env julia
# Fit the baseline (no-changepoint) COVID-19 SEIRD model to the smoothed data.
# Usage: julia fit_covid_base_model.jl <out_dir>
...
const x0 = [0.1633, 1/7, 1/11.4, 1/14, 1/13.4, 1/9, 1/16, 0.0055,
...
length(ARGS) >= 1 || error("Usage: julia fit_covid_base_model.jl <out_dir>")
const OUT_DIR = ARGS[1]
mkpath(OUT_DIR)

println("[base] Fitting baseline COVID model with $(Threads.nthreads()) threads...")
...
opt = OptimOptimizer(Fminbox(LBFGS()),
                     options=Optim.Options(show_trace=true, iterations=500, time_limit=7200.0))

loss, params = optimize_with_changepoints(
    Mica.objective_function, x0, parnames, Int[], (lower, upper), opt,
    n_global, n_segment_specific, model_manager, loss_abs_log, data_CP
)

println("[base] Best loss = $(round(loss, digits=4))")
println("[base] Parameters:")
for (i, p) in enumerate(parnames)
    println("  $p = $(params[i])")
end

CSV.write(joinpath(OUT_DIR, "covid_base_params.csv"),
          DataFrame(parameter=collect(string.(parnames)), value=params))

# Save D2D-compatible data file (log10(obs+1))
mkpath(joinpath(OUT_DIR, "Data"))
raw_data = max.(data_CP, 0.0)
log_data = log10.(raw_data' .+ 1.0)
data_df = DataFrame(
    time = 1.0:Float64(n),
    log_infected = log_data[:, 1],
    log_hospitalized = log_data[:, 2],
    log_icu = log_data[:, 3],
    log_death = log_data[:, 4],
    log_vaccinated = log_data[:, 5]
)
CSV.write(joinpath(OUT_DIR, "Data", "covid_data.csv"), data_df)

open(joinpath(OUT_DIR, "report.md"), "w") do f
    println(f, "# Baseline (no-CP) COVID model fit")
    println(f, "")
    println(f, "**Loss:** $(round(loss, digits=4))")
    println(f, "**Threads:** $(Threads.nthreads())")
    println(f, "")
    println(f, "| parameter | value |")
    println(f, "|---|---|")
    for (i, p) in enumerate(parnames)
        println(f, "| $p | $(params[i]) |")
    end
end

println("[base] Saved to $OUT_DIR")
