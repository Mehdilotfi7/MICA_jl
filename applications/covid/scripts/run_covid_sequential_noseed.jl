#!/usr/bin/env julia
# Sequential zero-penalty COVID run with no per-optimization seed.
# This is the closest configuration to the original pre-seed example code.
...
const OUT_DIR = joinpath(@__DIR__, "..", "..", "..", "results_sequential_noseed")
mkpath(OUT_DIR)
...
pop_size = 150
global_seed = length(ARGS) >= 1 ? parse(Int, ARGS[1]) : 1
...
opt = EvolutionaryOptimizer(ga,
        options=Evolutionary.Options(show_trace=false, iterations=1000), seed=nothing)

zero_penalty(p, n) = 0.0

println("\nRunning COVID objective = none (sequential, no per-opt seed, global_seed=$global_seed)")
...
label = "none"
CSV.write(joinpath(OUT_DIR, "covid_detected_cps_origset_$(label)_seed$(global_seed).csv"),
          DataFrame(objective=label, cp=detected_cp))

summary_file = joinpath(OUT_DIR, "covid_objectives_origset_summary_seed$(global_seed).json")
