#!/usr/bin/env julia
# Refit the SEIRD model for hand-picked / reported-NPI CP sets on a 500-day
# window (2020-01-27 -> 2021-06-11) and save fitted parameters + trajectories.
# Usage: julia simulate_handpicked_500.jl <label> <params_csv> <cps_csv> <cp_sets_dir> <out_dir>
...
# ---------- CLI ----------
length(ARGS) >= 5 || error("Usage: julia simulate_handpicked_500.jl <label> <params_csv> <cps_csv> <cp_sets_dir> <out_dir>")
const LABEL = ARGS[1]
...
function main()
    ...
    println("[simulate 500 $LABEL] Refitting + saving simulations for $n_jobs CP sets...")
    ...
    @sync for tid in 1:n_threads
        Threads.@spawn begin
            ...
            for i in start_idx:end_idx
                method = methods[i]
                cps = cp_sets[i]
                println("  [500 $LABEL][$tid] Simulating $method (n_cps=$(length(cps)))"); flush(stdout)
                t0 = time()
    ...
    println("[simulate 500 $LABEL] Done. Saved $(nrow(results)) simulations to $OUT_DIR/simulations/")
end
