#!/usr/bin/env julia
# Refit the SEIRD model for every alternative CP set produced by standard CPD methods.
# Output: revision/outputs/TASK_G/refit_summary.csv with fitted loss for each CP set.
...
const CP_SETS_DIR = joinpath(@__DIR__, "..", "..", "..", "outputs",  "TASK_G", "cp_sets")
const OUT_DIR = joinpath(@__DIR__, "..", "..", "..", "outputs",  "TASK_G")
mkpath(OUT_DIR)

# ---------- model / data (same as Task A/F) ----------
...
const params_best = Float64.(CSV.read(joinpath(TASK_A_DIR, "covid_params_origset_penalty_zero.csv"), DataFrame).value)
...
function main()
    ...
    # also include MICA zero-penalty set
    mica_cps = sort(unique(Int.(CSV.read(joinpath(TASK_A_DIR, "covid_detected_cps_origset_penalty_zero.csv"), DataFrame).cp)))
    if !use_subset || "MICA_zero_penalty" in subset_methods
        push!(methods, "MICA_zero_penalty")
        push!(cp_sets, mica_cps)
    end

    n_jobs = length(methods)
    println("[refit] Refitting $n_jobs CP sets...")

    results = DataFrame(method=String[], n_cps=Int[], cps=String[], refit_loss=Float64[], elapsed=Float64[], error=String[])
    # ensure results is thread-safe by pre-allocating locks
    lock_results = ReentrantLock()

    # Use threading; each refit is independent but ODE solves can allocate heavily,
    # so limit to a modest number of threads if many are available.
    ...
    CSV.write(joinpath(OUT_DIR, "refit_summary.csv"), results)
    println("[refit] Done. Summary saved to $(OUT_DIR)/refit_summary.csv")
    println(results)
end
