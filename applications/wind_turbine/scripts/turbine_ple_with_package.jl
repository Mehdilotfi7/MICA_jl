#!/usr/bin/env julia
# Profile-likelihood analysis for the wind-turbine application using
# BreakpointProfiles.jl.
#
# Usage: julia turbine_ple_with_package.jl <label> <cps_csv> <params_csv> <out_dir> [selection]
...
# CLI
length(ARGS) >= 4 || error("Usage: julia turbine_ple_with_package.jl <label> <cps_csv> <params_csv> <out_dir> [all|global|label1,label2,...]")
const LABEL = ARGS[1]
...
function main()
    ...
    println("[$LABEL] PLE for $(length(idxs)) parameter(s) out of $n_params"); flush(stdout)
    println("[$LABEL] CPs: $cps_best"); flush(stdout)
    println("[$LABEL] Best loss = $(round(best_loss, digits=4)); threshold = $(round(best_loss + CHISQ_95, digits=4))"); flush(stdout)
    ...
    lines = String[]
    push!(lines, "# Profile-likelihood analysis for wind-turbine `$(LABEL)`")
    push!(lines, "")
